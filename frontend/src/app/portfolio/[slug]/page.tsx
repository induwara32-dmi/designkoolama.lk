...
export default async function Page({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const fallback=portfolioProjects.find(item=>item.slug===slug);try{const project=await loadProject(slug,fallback);return <><JsonLd data={[creativeWorkJsonLd(project),breadcrumbJsonLd([{name:"Home",path:"/"},{name:"Portfolio",path:"/portfolio"},{name:project.title,path:`/portfolio/${project.slug}`}])]}/><CaseStudyPage project={project}/></>}catch(error){if(error instanceof Error&&error.message==="NOT_FOUND")notFound();return <ContentUnavailable/>}}
