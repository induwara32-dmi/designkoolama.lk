...
export function ContactForm(){const [feedback,setFeedback]=useState("");const {register,handleSubmit,formState:{errors,isSubmitting}}=useForm<Values>({resolver:zodResolver(schema)});const submit=async(values:Values)=>{setFeedback("");try{await contactSubmissionService.submit(values);}catch(error){setFeedback(error instanceof Error?error.message:"Unable to send your message.");}};return <form className="quote-card" noValidate onSubmit={handleSubmit(submit)}><div className="grid gap-6 md:grid-cols-2"><FormInput label="Full Name" placeholder="John Doe" error={errors.fullName?.message}{...register("fullName")}/><FormInput label="Email Address" type="email" placeholder="john@company.com" error={errors.email?.message}{...register("email")}/><FormInput label="Phone Number" type="tel" placeholder="+94 77 123 4567" error={errors.phone?.message}{...register("phone")}/><FormInput label="Subject" placeholder="How can we help?" error={errors.subject?.message}{...register("subject")}/></div><div className="mt-6"><FormTextarea label="Message" placeholder="Tell us about your project..." error={errors.message?.message}{...register("message")}/></div>{feedback&&<div className="mt-5"><ErrorMessage message={feedback}/></div>}<span className="sr-only"><SuccessMessage message="Message sent"/></span><Button className="mt-7 w-full uppercase tracking-widest" disabled={isSubmitting}>{isSubmitting?<LoadingIndicator label="Sending"/>:"Send Message"}</Button></form>}
